...
class CollectionAccountWiseController extends Controller
{
  
    public function index(Request $oRequest)
    {
    
        $data['page_title'] = 'Account collection Summary List';
        ...
        $rows = $query->where('delete_status','0')
                            ...
                            ->groupBy('tran_page_type')
                            ->selectRaw('*, sum(amount) as total_amount')
                            ->orderBy('id', 'DESC')->get();
        //return $rows;                    
        return view('admin.templates.CollectionAccountWise.CollectionAccountWise',compact('data','branches','rows','TrialBalanaces'));
    }
