...
@endsection
    ...
    @section('content')
        ...
        @if(Request::route()->getName() == 'admin.loanScheme.index') 
            @include('admin.templates.loanScheme.list')
        @endif

        @if(Request::route()->getName() == 'admin.loanScheme.create') 
            @include('admin.templates.loanScheme.add')
        @endif

        @if(Request::route()->getName() == 'admin.loanScheme.edit') 
            @include('admin.templates.loanScheme.update')
        @endif

        @if(Request::route()->getName() == 'admin.loanScheme.view') 
            @include('admin.templates.loanScheme.manage')
        @endif
...
{
  var API_URL = "{{ route('admin.loanScheme.list') }}";
    $.ajax({
     url: API_URL,
     method: 'get',
     data: {},
...
function add_row(form)
{
     ...
     var API_URL = "{{ route('admin.loanScheme.store') }}";
     ...
     function update_row()
     {
         ...
         var formData = new FormData(form);
         var API_URL = "{{ route('admin.loanScheme.update') }}";
            $.ajax({
                url: API_URL,
                 type: 'POST',
                 data: formData,
      ...
      Swal.fire({
        ...
        if(t.value == true){ 
            var API_URL = "{{ route('admin.loanScheme.delete') }}";
            $.ajax({
                type: "POST",
                url: API_URL,
  ...
  function delete_panelty_row(update_id) {
      ...
      Swal.fire({
        ...
        if(t.value == true){ 
            var API_URL = "{{ route('admin.loanScheme.panelty-delete') }}";
            $.ajax({
                type: "POST",
                url: API_URL,
                data: {'update_id': update_id,'_token': '{!! csrf_token() !!}'},
