...
@endsection
    ...
    @section('content')
        ...
        @if(Request::route()->getName() == 'admin.recurringScheme.index') 
            @include('admin.templates.recurringScheme.list')
        @endif

        @if(Request::route()->getName() == 'admin.recurringScheme.create') 
            @include('admin.templates.recurringScheme.add')
        @endif

        @if(Request::route()->getName() == 'admin.recurringScheme.edit') 
            @include('admin.templates.recurringScheme.update')
        @endif

        @if(Request::route()->getName() == 'admin.recurringScheme.view') 
            @include('admin.templates.recurringScheme.manage')
        @endif
...
{
  var API_URL = "{{ route('admin.recurringScheme.list') }}";
    $.ajax({
     url: API_URL,
     method: 'get',
     data: {},
...
function add_row(form)
{
     ...
     var API_URL = "{{ route('admin.recurringScheme.store') }}";
     ...
     function update_row()
     {
         ...
         var formData = new FormData(form);
         var API_URL = "{{ route('admin.recurringScheme.update') }}";
            $.ajax({
                url: API_URL,
                 type: 'POST',
                 data: formData,
      ...
      Swal.fire({
        ...
        if(t.value == true){ 
            var API_URL = "{{ route('admin.recurringScheme.delete') }}";
            $.ajax({
                type: "POST",
                url: API_URL,
