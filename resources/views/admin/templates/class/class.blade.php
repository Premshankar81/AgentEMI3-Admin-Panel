...
@endsection
    ...
    @section('content')
        ...
        @include('admin.templates.class.list')


        @include('admin.templates.class.add')
        @include('admin.templates.class.update')
...
{
  var API_URL = "{{ route('admin.class.list') }}";
    $.ajax({
     url: API_URL,
     method: 'get',
     data: {},
...
function add_row(form)
{
     ...
     var API_URL = "{{ route('admin.class.store') }}";
    ...
    function get_row(id)
    {
     var API_URL = "{{route('admin.class.get')}}";
        $.ajax({
            url: API_URL,
            type: 'POST',
           data: {'id': id,'_token': '{!! csrf_token() !!}'},
            ...
            success: function(data) 
            {
              if (data["status"] == 1){
                 ...
                 $('#update_class_fee').val(data["data"]["class_fee"]);
     ...
     function update_row()
     {
         ...
         var formData = new FormData(form);
         var API_URL = "{{ route('admin.class.update') }}";
            $.ajax({
                url: API_URL,
                 type: 'POST',
                 data: formData,
      ...
      Swal.fire({
        ...
        if(t.value == true){ 
            var API_URL = "{{ route('admin.class.delete') }}";
            $.ajax({
                type: "POST",
                url: API_URL,
