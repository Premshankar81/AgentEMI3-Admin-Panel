...
@endsection
    ...
    @section('content')
        ...
        @include('admin.templates.state.list')

        @include('admin.templates.state.add')
        @include('admin.templates.state.update')
...
{
  var API_URL = "{{ route('admin.state.list') }}";
    $.ajax({
     url: API_URL,
     method: 'get',
     data: {},
...
function add_row(form)
{
     ...
     var API_URL = "{{ route('admin.state.store') }}";
    ...
    function get_row(id)
    {
     var API_URL = "{{route('admin.state.get')}}";
        $.ajax({
            url: API_URL,
            type: 'POST',
           data: {'id': id,'_token': '{!! csrf_token() !!}'},
            ...
            success: function(data) 
            {
              if (data["status"] == 1){
                 ...
                 $('#update_county_id').val(data["data"]["country_id"]);
                 
                 $('#update_status').val(data["data"]["status"]);
                 $('#GetTypeCheckBox').html(data["html"]);
                 selectPickerInitialize('update_county_id');
     ...
     function update_row()
     {
         ...
         var formData = new FormData(form);
         var API_URL = "{{ route('admin.state.update') }}";
            $.ajax({
                url: API_URL,
                 type: 'POST',
                 data: formData,
    ...
    if(t.value == true){ 
        var API_URL = "{{ route('admin.state.delete') }}";
        $.ajax({
            type: "POST",
            url: API_URL,
