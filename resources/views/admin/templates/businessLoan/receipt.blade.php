...
<ol class="breadcrumb">
    ...
    <li><a href="{{route('admin.businessLoan.manage',array('id' => $row['loan_account']['uuid']))}}">Loan Account - {{$row->loan_account->account_no}}</a></li>
...
@if(!empty(Auth::guard('admin')->user()->pdf_logo))
<img src="{{config('constants.SYSTEM')}}{{Auth::guard('admin')->user()->pdf_logo}}" class="img" style="max-width:180px;max-height:120px;" style="margin: 0 auto;width:100%;height:100%;"  />
@else
<img src="{{config('constants.DEFAULT_IMAGE')}}" class="img" style="max-width:180px;max-height:120px;" style="margin: 0 auto;width:100%;height:100%;"  />
@endif
</div>
...
<table style="width:100%;font-size: 11px; font-family:Calibri;">
    <tbody>
        ...
        <tr>
            <td>A/c No</td>
            <td>:</td>
            <td style="text-align: right;">{{@$row->loan_account->account_no}}</td>
        </tr>
    </tbody>
</table>
